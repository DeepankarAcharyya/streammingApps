...
func sendToStream(csi_ Services_ChatServiceServer, clientUniqueCode_ int, errch_ chan error) {
	...
	for {
		...
		for {
			...
			messgae4Client := messageHandleObject.MQue[0].MessageBody

			messageHandleObject.mu.Unlock()

			//send message to the designated client (do not send to the same client)
			if senderUniqueCode != clientUniqueCode_ {
				err := csi_.Send(&FromServer{Name: senderName4Client, Body: messgae4Client})
				...
				if len(messageHandleObject.MQue) > 1 {
					//delete the message at index 0 after sending it to the client
